<?php 
    ...
    $fullURL = $_SERVER['REQUEST_URI']; // Gets the full URL, e.g., "/about.php"
    $parts = explode('/', $fullURL);     // Split the URL by slashes
    $route = end($parts);               // Get the last part of the URL, which is the route or filename
...
<head>
    ...
    <?php if($route=='about-us.php'){ ?>
      ...
      <?php }else if($route=='industry.php'){?>   
        ...
        <?php } ?>
        <?php if($route=='enterprise-safety.php'){
          ?>
          <link rel="stylesheet" href="enterprisesafetyplatfrom.css">
          <?php
        }?>
                <?php if($route=='personal-safety-app.php'){
          ?>
          <link rel="stylesheet" href="personalsafetyapp.css">
          <?php
        }?> 
        <?php if($route=='journey-management.php'){
          ?>
